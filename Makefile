...
# Build, install, and launch the iOS demo on a connected iOS device.
...
ios_dev:
	@if [ -z "$(DEVICE_ID)" ]; then \
		...
		echo "    and samples/iosApp/Configuration/Config.local.xcconfig must set TEAM_ID."; \
		exit 1; \
	fi
	...
	xcodebuild \
		...
		build
	@APP="$$(ls -d samples/iosApp/build/Build/Products/Debug-iphoneos/*.app | head -n1)"; \
	BUNDLE_ID="$$(/usr/libexec/PlistBuddy -c 'Print :CFBundleIdentifier' "$$APP/Info.plist")"; \
	xcrun devicectl device install app --device "$(DEVICE_ID)" "$$APP"; \
	xcrun devicectl device process launch --device "$(DEVICE_ID)" "$$BUNDLE_ID"

# Build, install, and launch the iOS demo on the booted iOS simulator.
...
ios_dev_sim:
	...
	xcodebuild \
		...
		-derivedDataPath samples/iosApp/build \
		build
	@APP="$$(ls -d samples/iosApp/build/Build/Products/Debug-iphonesimulator/*.app | head -n1)"; \
	BUNDLE_ID="$$(/usr/libexec/PlistBuddy -c 'Print :CFBundleIdentifier' "$$APP/Info.plist")"; \
