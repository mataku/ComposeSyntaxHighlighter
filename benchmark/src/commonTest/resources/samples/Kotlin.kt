...
class TaskRepository : Repository<Task> {
  ...
  fun filterByPriority(priority: Priority): List<Task> = tasks.filter { it.priority == priority }

  fun filterByTag(tag: String): List<Task> = tasks.filter { it.tags.contains(tag) }
  ...
  fun sortByDueDate(): List<Task> = tasks.sortedWith(compareBy(nullsLast()) { it.dueDate })
...
fun TaskRepository.highPriority(): TaskList = filterByPriority(Priority.HIGH)
...
fun main() {
  ...
  repo.add(
    ...
    Priority.HIGH,
    listOf("shopping"),
    LocalDate.parse("2024-12-25", fmt),
  )
  ...
  repo.add(
    ...
    Priority.LOW,
    listOf("health"),
    LocalDate.parse("2024-11-01", fmt),
  )
